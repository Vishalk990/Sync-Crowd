  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(
        "https://suryanshbachchan.us-east-1.modelbit.com/v1/generateDataUsingLLM/latest",
        {
              ...
              {data.map((row, index) => (
                <tr key={index}>
                  {Object.values(row).map((value, i) => (
                    <td
                      key={i}
                      className="border border-gray-300 p-2 break-words"
                    >
                      {value}
                    </td>
                  ))}
